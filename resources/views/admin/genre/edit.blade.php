    ...
    <div class="ad_displayDetails col-10 d-flex">
        <div class="author_form w-full flex justify-center items-center">
            <div class="author_from rounded-lg shadow-md p-8 max-w-sm w-full bg dark:bg-gray-800">
                ...
                <form method="POST" action={{route("admin.genre.update",['id'=> $genre->id])}} class="space-y-4">
                    @csrf
                    @method('PUT')
                    <x-input-label for="name" :value="__('Genre Name')" />
                    <x-text-input value="{{$genre->name}} " id="name" name="name" type="text" class="mt-1 block w-full"  />
